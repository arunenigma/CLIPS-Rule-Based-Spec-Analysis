           ...
           (if (and (eq (length$ $?slots) 1)
            ...
            then ; Handle implied deftemplates
              ...
              (assert-string (str-cat "(" ?template " "
                                 (decode-string-delimiters (sub-string 0 (str-length ?value) ?value))
                      ")"))
            else
              ...
              (progn$ (?value (rest$ $?values))
                (if (deftemplate-slot-multip ?template (sym-cat (nth$ ?value-index $?slots)))
                 then ; Multislots
               		(bind ?fact (str-cat ?fact " (" (nth$ ?value-index $?slots) " " (decode-string-delimiters (sub-string 0 (str-length ?value) ?value)) ")"))
                 else
